...
function preload() {
  ...
  plutoAudio = loadSound("audio/BTS-Pluto.mp3");
  sweetChaosAudio = loadSound("audio/DAY6-SweetChaos.mp3");
  instagramAudio = loadSound("audio/DEAN-Instagram.mp3");
  rainbowAudio = loadSound("audio/KERMIT-RainbowConnection.mp3");
  psychoAudio = loadSound("audio/REDVEL-Psycho.mp3");
  unforgettableAudio = loadSound("audio/SIA-Unforgettable.mp3");
  astronautAudio = loadSound("audio/SKZ-Astronaut.mp3");
  getCoolAudio = loadSound("audio/SKZ-GetCool.mp3");
  insomniaAudio = loadSound("audio/SKZ-Insomnia.mp3");
  sunshineAudio = loadSound("audio/SKZ-Sunshine.mp3");
...
function showMusic() {
  musicWindow = true;
  musicExitBtn.show();
  rect(150, 100, 400, 200); //whole window
  rect(150, 100, 60, 200); //side menu
  rect(150, 100, 400, 10); //top bar
  ...
  push();
    ...
    fill(0);
    textSize(7);
    text("Piano Covers", 153, 120);
  pop();

  push();
    rect(330, 125, 100, 100); //cover art position
    image(songCovers[currSongIndex], 330, 125, 100, 100);
    textAlign(CENTER);
    textSize(10);
    fill(0);
    text(songNames[currSongIndex], 380, 240);
    textSize(7);
    text(songArtist[currSongIndex], 380, 250);
  pop();
...
function pauseAndPlay() {
  counter++;
  image(skipBackImg, 330, 260, 25, 25);
  image(skipForwardImg, 430-25, 260, 25, 25);
  if(audioPlaying) {
    image(pauseImg, 365, 257, 30, 30);
  }
  else {
    image(playImg, 365, 257, 30, 30);
  }

  if(!paintWindow) {
    //pause/play clicked
    if(mouseIsPressed && mouseX > 365 && mouseX < 365+30 && mouseY > 257 && mouseY < 257+30 && counter > 10) {
      if(audioPlaying) {
    ...
    //back clicked
    if(mouseIsPressed && mouseX > 330 && mouseX < 330+25 && mouseY > 260 && mouseY < 260+25 && counter > 10) {
      if(currSongIndex == 0) {
    ...
    //forward clicked
    if(mouseIsPressed && mouseX > 405 && mouseX < 430 && mouseY > 260 && mouseY < 260+25 && counter > 10) {
